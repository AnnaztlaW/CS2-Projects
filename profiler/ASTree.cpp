...
void AST::mainHeader(const std::vector<std::string>& profileName,
                     const std::vector<std::string>& fileName     ) { //done
    //TODO: IMPLEMENT
    //Skip down a couple lines (in fileName) or find main and put it before it
    ...
    //insert after root, as a child
    //loop is needed for profile names

    std::list<AST*>::iterator ptr = child.begin();
    std::string line="";

     //insert will push everything down so add the #include last
...
void AST::mainReport(const std::vector<std::string>& profileName) {
    ...
    //main function will be child of root
    for(std:: list<AST*>::const_iterator i= child.begin(); i !=child.end(); ++i){
        AST* mainNode = *i;
        if(mainNode->tag == "function"){         //find functions
            
            //declare node = getChild("name")
            if(mainNode->getChild("name")->getName() == "main"){       //find main
                //find node "block_content"
                AST* blockNode= mainNode->getChild("block")->getChild("block_content");  //children of main

                // list backward to find last return statement (iterator will point at return)
                std:: list<AST*>::iterator j= blockNode->child.end();
                j--;
                j--;
                    //insert lines (iterate over vector to print all file names)
                for(size_t k=0; k < profileName.size(); ++k){         //add lines before return
                        std::string line = "std::cout << " + profileName[k]+ " << std::endl; \n    ";
                        child.insert(j, new AST(token, line));
                }
...
void AST::functionCount(const std::string& profileName) {
    ...
    for (std::list<AST*>::const_iterator i = child.begin(); i != child.end(); ++i) {
        if(((*i)->tag == "function") ){
            std::string nameFunc = (*i)->getChild("name")->getName();
            std:: list<AST*>::iterator place= ((*i)->getChild("block")->getChild("block_content"))->child.begin();

            std::string line =  "      " + profileName + ".count(__LINE__, \"" + nameFunc + "\"); \n" ;
            child.insert(place, new AST(token, line));
...
void AST::lineCount(const std::string& profileName) {
    ...
    for (std::list<AST*>::const_iterator i = child.begin(); i != child.end(); ++i) {
        if ((*i)->nodeType == category){
            //recursion over the children of roots children
            if(tag == "expr_stmt"){
                std:: list<AST*>::iterator place= (*i)->child.end();
                //place++;
