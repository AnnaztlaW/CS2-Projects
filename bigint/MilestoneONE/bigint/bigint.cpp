...
//previously I only have a commit as a submission in the bigint file
//here is a explict version for milestone one
...
int bigint::operator[](int index)const{
   int sizeNumber=0;
   int i=maximumSize;
   while(i>=0 && maximumSize==0){
      if(number[i]>0)
         sizeNumber+=i;
      i--;
   }
   if(index<=sizeNumber){
      return number[sizeNumber-index];
   }
   else
      return -1;
}
